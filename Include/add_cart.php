...
require 'mainpageconnect.php';
// $mysqli = new mysqli("localhost", "root", "toor", "Giftit");

$product_id=real_escape_string($_POST['product_id']);
$size=real_escape_string($_POST['size']);
$available=real_escape_string($_POST['available']);
$quantity=real_escape_string($_POST['quantity']);
$item=array();
$item[]=array(
'id'=>$product_id,
'size'=>$size,
...
if($cart_id != ''){
	...
	$cart_expire=date("Y-m-d H:i:s",strtotime("+30 days"));
	$mysqli->query("INSERT INTO cart(Items,expire_date)VALUES ('{$item_json}','{$cart_expire}')");
